...
def main():
    ...
    while True:
		...
		# tương tự như phần sinh viên ở trên
            ...
            course_id = input("Nhập mã khóa học thêm vào: ")
            course_name = input("Nhập tên khóa học thêm vào: ")
            description = input("Nhập mô tả khóa học thêm vào: ")
            credits = input("Nhập số tín chỉ khóa học thêm vào: ")  
            course = Course(course_id, course_name, description, credits)
            course.add_course(db)

        elif choice == "6":
            course_id = input("Nhập mã khóa học cần chỉnh sửa: ")
            course = Course.search_course(db, course_id)
            if course:
                course_name = input("Nhập tên khóa học mới: ")
                description = input("Nhập mô tả khóa học mới: ")
                credits = input("Nhập số tín chỉ khóa học mới: ")
